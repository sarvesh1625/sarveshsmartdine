...
export default function CartDrawer({ open, onClose, restaurant, slug, tableId, upiId, upiOnly = false }) {
  ...
  const { items, updateQuantity, clearCart, getTotal, restaurantId } = useCartStore();
  ...
  async function payViaMerchant() {
    ...
    } catch { toast.error('Payment error. Scan QR to pay.'); }
  ...
  function handleClose() {
    if (step === 'success' || step === 'pay') resetDrawer();
    onClose();
  }
  ...
  // Core place-order function — accepts name/phone directly to avoid async state issue
  async function placeOrder(customerName, customerPhone, instructions) {
    setLoading(true);
    try {
  ...
  async function proceedToPayment() {
    ...
    await placeOrder(form.name.trim(), cleanPhone, form.instructions);
  }

  // Called when customer already signed in — skip details step
  async function proceedDirectly() {
    const name  = customer.name.trim();
    const phone = customer.phone.replace(/\D/g, '').slice(-10);
    // Pre-fill form so details step shows correct info if user goes back
    setForm(f => ({ ...f, name, phone }));
    await placeOrder(name, phone, form.instructions);
  }
  ...
  const isSignedIn = !!(customer?.name && customer.name !== 'Guest' && customer?.phone);
            ...
            <div className="flex-1 overflow-y-auto">
                    ...
                    <div className="px-5 py-4 space-y-3">
                        ...
                        <div key={item.id} className="flex items-center gap-3 bg-white/5 rounded-2xl p-3">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="w-7 h-7 rounded-full bg-white/10 text-white flex items-center justify-center text-sm hover:bg-white/20"
                            >−</button>
                            <span className="text-white font-bold text-sm w-4 text-center">{item.quantity}</span>
                            <button
                              onClick={() => updateQuantity(item.id, item.quantity + 1)}
                              className="w-7 h-7 rounded-full bg-[#e94560] text-white flex items-center justify-center text-sm hover:bg-[#d63050]"
                            >+</button>
                          </div>
                        </div>
                      ))}
                      ...
                      {couponApplied && (
                        <div className="flex items-center justify-between bg-green-500/10 border border-green-500/20 rounded-xl px-3 py-2">
                          <span className="text-green-400 text-xs font-bold">✓ {couponApplied.code} applied</span>
                          <button onClick={() => setCouponApplied(null)} className="text-white/30 text-xs hover:text-red-400">Remove</button>
                        </div>
                      )}
                      ...
                      {/* Show signed-in customer info */}
                      {isSignedIn && (
                        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5">
                          <div className="w-6 h-6 rounded-full bg-[#e94560]/20 flex items-center justify-center text-[#e94560] font-black text-xs flex-shrink-0">
                            {customer.name[0].toUpperCase()}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-white text-xs font-bold truncate">{customer.name}</p>
                            <p className="text-white/30 text-xs">{customer.phone}</p>
                          </div>
              ...
              {step === 'details' && (
                <div className="px-5 py-5 space-y-4">
                  ...
                  {/* Payment method */}
                  <div>
                    <label className="text-white/50 text-xs font-semibold uppercase tracking-wider mb-3 block">
                      Payment Method
                    </label>
                    {upiOnly ? (
                      <div className="bg-[#e94560]/10 border border-[#e94560]/25 rounded-xl px-4 py-3 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-white/40 text-xs mt-0.5">This table requires UPI payment</p>
                        </div>
                    ...
                    {(payMethod === 'upi' || upiOnly) && (
                      <div className="mt-3 bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-3 flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-white/35 text-xs mt-0.5">Scan QR with any UPI app</p>
                        </div>
                      </div>
                    )}
                  ...
                  {hasMerchant ? (
                    <>
                      <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-center w-full mb-1">
                        ...
                        <p className="text-white/35 text-xs mt-0.5">Payment confirms automatically</p>
                      </div>
                      <button
                        onClick={payViaMerchant}
                        disabled={loading}
                        className="w-full text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 mb-2"
                    ...
                    <div className="bg-yellow-500/8 border border-yellow-500/15 rounded-xl p-3 text-center w-full mb-2">
                      <p className="text-yellow-400 text-xs">🧾 Show payment screenshot to staff after paying</p>
                    </div>
                  )}

                  <button
                    onClick={() => navigate(`/order/${slug}/${orderId}`)}
                    className="w-full border border-white/10 text-white/50 font-semibold py-3 rounded-2xl text-sm mt-1"
                  >Track Order →</button>
                  <button
                    onClick={() => { handleClose(); navigate(`/my-orders/${slug}`); }}
                    className="w-full border border-white/10 text-white/30 font-semibold py-3 rounded-2xl text-sm mt-2"
              ...
              {step === 'success' && (
                <div className="px-5 py-10 text-center">
                  ...
                  {orderId && (
                    <button
                      onClick={() => { handleClose(); navigate(`/order/${slug}/${orderId}`); }}
                  ...
                  <button
                    onClick={handleClose}
                    className="w-full border border-white/10 text-white/30 font-semibold py-3 rounded-2xl"
                  >Back to Menu</button>
                </div>
              )}
            </div>
            ...
            <div className="px-5 py-4 border-t border-white/5">
              {step === 'cart' && items.length > 0 && (
                <button
                  onClick={() => {
                    if (isSignedIn) {
                      // Skip details — customer already signed in
                      proceedDirectly();
                    } else {
                      setStep('details');
                    }
                  ...
                  disabled={loading}
                  className="w-full bg-[#e94560] hover:bg-[#d63050] disabled:opacity-50 text-white font-bold py-4 rounded-2xl flex items-center justify-between px-6 shadow-lg shadow-[#e94560]/25"
                >
                  {loading ? (
                    ...
                    <>
                      <span>{isSignedIn ? 'Place Order' : 'Proceed to Checkout'}</span>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {loading
                      ...
                      : (upiOnly || payMethod === 'upi') ? `Pay ₹${total.toFixed(0)} →` : `Place Order · ₹${total.toFixed(0)}`
                    }
                  </button>
                </div>
              )}
